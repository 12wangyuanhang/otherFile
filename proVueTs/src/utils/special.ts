...
const _loveSpecial = (e: any, t: any, a?: any) => {
    ...
    e.requestAnimationFrame = e.requestAnimationFrame || e.webkitRequestAnimationFrame || e
        .mozRequestAnimationFrame || e.oRequestAnimationFrame || e.msRequestAnimationFrame || function (e: any) {
            setTimeout(e, 1e3 / 60)
        }, i(
...
function fairyDustCursor() {
    ...
    function Particle<T>(this: any): any {
    ...
    function applyProperties(target: HTMLSpanElement, properties: any) {
        for (var key in properties) {
            (target.style as { [key: string]: any })[key] = properties[key];
        }
    }
...
function _getSakura() {
    ...
    function startSakura() {
        ...
        var sakuraList = new (<any>SakuraList)();
        for (var i = 0; i < 50; i++) {
    ...
    window.onresize = function () {
        var canvasSnow: any = document.getElementById('canvas_snow');
...
function bubblesCursor() {
    ...
    function Particle(this: any) {
        ...
        // Init, and set properties
        this.init = function (x: any, y: any) {
    ...
    const colours = ["#e6f1f7", "#b7dcf1", "#949cff", "#e4c1ff", "#613093"];
    /**
     * Utils
     */
    ...
    function applyProperties(target: any, properties: any) {
        for (var key in properties) {
            ...
            target.style.background = colours[~~(Math.random()*5)]
        }
    }

    init();
}
export const getBubblesCursor = () => {
    cancelSpecial()
    bubblesCursor()
}
...
function emojiCursor() {
    ...
    function bindEvents() {
        ...
        document.ontouchstart = onTouchMove;
    ...
    function Particle(this:any) {
        ...
        // Init, and set properties
        this.init = function (x:number, y:number, character:any) {
...
export const getEmojiCursor = () => {
    cancelSpecial()
    emojiCursor()
}
// 蜘蛛网特效
function spiderEffects() {
    function n(n:any, e:any, t:any) {
    ...
    function i() {
        r.clearRect(0, 0, a, c);
        var n, e, t, o, m, l;
        s.forEach(function (i:any, x:any) {
            for (i.x += i.xa, i.y += i.ya, i.xa *= i.x > a || i.x < 0 ? -1 : 1, i.ya *= i.y > c || i.y < 0 ? -1 : 1, r.fillRect(i.x - .5, i.y - .5, 1, 1), e = x + 1; e < u.length; e++) n = u[e], null !== n.x && null !== n.y && (o = i.x - n.x, m = i.y - n.y, l = o * o + m * m, l < n.max && (n === y && l >= n.max / 2 && (i.x -= .03 * o, i.y -= .03 * m), t = (n.max - l) / n.max, r.beginPath(), r.lineWidth = t / 2, r.strokeStyle = "rgba(" + d.c + "," + (t + .2) + ")", r.moveTo(i.x, i.y), r.lineTo(n.x, n.y), r.stroke()))
        }), x(i)
    }

    var a:any, c:any, u:any, m = document.createElement("canvas"), d = t(), l = "c_n" + 2, r = m.getContext("2d") as any,
        // @ts-ignore
        x = window.requestAnimationFrame || window.webkitRequestAnimationFrame || window.mozRequestAnimationFrame || window.oRequestAnimationFrame || window.msRequestAnimationFrame || function (n) {
            window.setTimeout(n, 1e3 / 45)
        }, w = Math.random, y:any = { x: null, y: null, max: 2e4 };
    m.id = l, m.style.cssText = "position:fixed;top:0;left:0;z-index:" + d.z + ";opacity:" + d.o, e("body")[0].appendChild(m), o(), window.onresize = o, window.addEventListener('mousemove',function (n) {
...
function clearEffects(){
    if (document.getElementById('canvas_sakura')) {
        console.log(123145)
        let canvas_sakura: any = document.getElementById('canvas_sakura');
        document.body.removeChild(canvas_sakura)
    }
    if (document.getElementById('c_n2')) {
        let canvas_sakura: any = document.getElementById('c_n2');
        document.body.removeChild(canvas_sakura)
    }
}
export const getSpiderEffects = () => {
    clearEffects();
    spiderEffects()
}