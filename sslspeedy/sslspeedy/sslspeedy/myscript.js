...
/*https://github.com/gosquared/flags/blob/master/flags/flags/flat/24/United-Kingdom.png*/
...
function saveBlackDomainList(){
 //get all domains from blackdomain form
  ...
  var url="https://678in.com/?action=saveBlackDomainList&white=1&username="+localStorage['username']+"&password="+localStorage['password']+"&domains="+domains;
  $.getScript(url);


}
function reloadServers(){
    $.getScript("https://678in.com/?action=serverlistjson&callback=loadServerlist");
}
...
function connect(){
    ...
    $("#cinfo").html( "connecting...");
     url = ("https://678in.com/sslspeedy.php?login=1&white=1&callback=setproxy&user="+$('#username').val()+"&pass="+$('#password').val());
     $.getScript(url);
     reloadServers();
...
function setproxy(result){
  var defaultbypassdomains ="taobao.com";
  if(isReconnect==0){
...
function setChromeProxy(blkdomains,wlistdomains,httpsserver){
      ...
      var config = {
       ...
       " }\n"+
        ...
        "var proxy_domains=["+blkdomains+"'taobao.com'];\n"+
     ...
     var auto_black = getItem('localbldomain').split("\n");
      ...
      chrome.tabs.query({'active': true, 'windowId': chrome.windows.WINDOW_ID_CURRENT},
              function(tabs){
                  ...
                  var currentdomainurl =  pageurl[2];

                  if(  currentdomainurl.indexOf(".")>0 ){
                      currentdomainurl = currentdomainurl.split(".")
                     var currentdomain = currentdomainurl[(currentdomainurl.length-2)]+"."+currentdomainurl[(currentdomainurl.length-1)];


                      i = 0;
                      gotwhitelist = 0;
                      while(i< auto_black.length) {

                           if( auto_black[i].length<3){
                            i++;
                            continue;
                          }
                           ...
                           if( dnsDomainIs(currentdomain,auto_black[i] ) ) {
                              $("#currentBrowsingDomain").val( currentdomain);
                              $("#sslremovebt").show();
                              $("#ssladdbt").hide();
                              $("#domainstatus").html("白名单直连");
                              chrome.browserAction.setIcon({
                                    path: "icon-ok.png"
                                });
                              gotwhitelist=1;
